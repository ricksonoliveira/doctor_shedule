defmodule DoctorScheduleWeb.Api.AppointmentController do
  ...
  def index(conn, _params) do
    appointments = AppointmentRepository.list_appointments()
  ...
  def show(conn, %{"id" => id}) do
    appointment = AppointmentRepository.get_appointment!(id)
  ...
  def update(conn, %{"id" => id, "appointment" => appointment_params}) do
    ...
    with {:ok, %Appointment{} = appointment} <-
           AppointmentRepository.update_appointment(appointment, appointment_params) do
      render(conn, "show.json", appointment: appointment)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %Appointment{}} <- AppointmentRepository.delete_appointment(appointment) do
